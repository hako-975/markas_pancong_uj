<?php 
    ...
    $dataPesanan = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM pesanan INNER JOIN user ON pesanan.id_user=user.id_user WHERE kode_pesanan = '$kode_pesanan'"));

    $detail_pesanan = mysqli_query($koneksi, "SELECT * FROM detail_pesanan INNER JOIN menu ON detail_pesanan.id_menu = menu.id_menu INNER JOIN pesanan ON detail_pesanan.kode_pesanan = pesanan.kode_pesanan INNER JOIN user ON pesanan.id_user = user.id_user WHERE detail_pesanan.kode_pesanan = '$kode_pesanan'");
...
<head>
    <title>Detail Pesanan - <?= $dataPesanan['nama_lengkap']; ?></title>
            ...
            <div class="card-header py-3">
                <div class="row">
                    <div class="col head-left">
                        <h6 class="mt-2 font-weight-bold text-primary">Detail Pesanan - <?= $dataPesanan['nama_lengkap']; ?></h6>
                    </div>
                                            ...
                                            <form method="post" enctype="multipart/form-data">
                                                ...
                                                <div class="modal-content">
                                                  <div class="modal-header">
                                                    <h5 class="modal-title" id="ubahPesananModalLabel<?= $dp['id_detail_pesanan']; ?>"><i class="fas fa-fw fa-edit"></i> Ubah Pesanan - <?= $dp['nama_lengkap']; ?></h5>
